...
int isTmax(int x) {
	int t = x + 1;
	int neg_t = (~t + 1);
	//in 32-bit integer, the additive inverse of 0x80000000 is itself.
	//but the additive inverse of 0x0 is also itself.
	int self_inv = !(t ^ neg_t);
	int nz = !!t;
...
int conditional(int x, int y, int z) {
	int cond = !x;
	int mask = ~0 + cond;
	//if x is not zero, then mask will remain 0xffffffff;
...
int logicalNeg(int x) {
	int top = x | (~x + 1);
	//the most significant bit will remain 0 if x is zero.
	//otherwise the most significante will be 1.
	return (top >> 31) + 1;
}
