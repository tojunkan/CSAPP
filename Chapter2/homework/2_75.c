...
{
        unsigned x, y;
	printf("please enter two unsigned numbers x and y. the program will return the highest w bits of the product.\n");
	scanf("%u%u", &x, &y);
        show_bits((byte_pointer)&x, sizeof(unsigned));
        show_bits((byte_pointer)&y, sizeof(unsigned));
	printf("\n");
	unsigned ans = unsigned_high_prod_optimized(x, y);
	int a, b;
	printf("please enter two int numbers a and b. the program will return the highest w bits of the product.\n");
	scanf("%d%d", &a, &b);
        show_bits((byte_pointer)&a, sizeof(int));
        show_bits((byte_pointer)&b, sizeof(int));
	printf("\n");
	int res = signed_high_prod_optimized(a, b);

	printf("\n\nnote that the type int and unsigned is convertable, \nso the unsigned number x and y is actually %d and %d when intepreted as int;\nand the int number a and b is actually %u and %u when intepreted as unsigned.\n", (int)x, (int)y, (unsigned)a, (unsigned)b);
	//actually, the answer of unsigned/signed function has some relationship.
	//it's due to the following formula:
	//x'=x+x_31 * 2^32, y'=y+y_31 * 2^32, where
	//x is the signed form of a bit sequence, and x' is the unsigned one; same with y.
	//so the product can be written as:
	//x' * y' = x*y + (x*y_31+y*x_31)*2^32 + x_31*y_31*2^64.
	//however, we only consider the least significant 64 bits (which is to 2^63), so the difference is x*y_31 + y*x_31.
	printf("\n\n");
	printf("variations:\nthe highest w bits of the int version of x and y is:\n");
	unsigned xsgn = !!(x&0x80000000), ysgn = !!(y&0x80000000);
	int intans = (int)(ans - (xsgn?y:0) - (ysgn?x:0));
	show_bits((byte_pointer)&intans, sizeof(int));
	printf("use function signed_high_prod_optimized to check if it's true:\n");
	signed_high_prod_optimized((int)x, (int)y);
	printf("\n\n");
	printf("variations:\nthe highest w bits of the unsigned version of a and b is:\n");
	int asgn = !!(a&0x80000000), bsgn = !!(b&0x80000000);
	unsigned unsignedres = (unsigned)res + (asgn?(unsigned)b:0) + (bsgn?(unsigned)a:0);
	show_bits((byte_pointer)&unsignedres, sizeof(int));
	printf("use function unsigned_high_prod_optimized to check if it's true:\n");
	unsigned_high_prod_optimized((unsigned)a, (unsigned)b);
	return 0;
}
